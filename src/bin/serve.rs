...
use nstar_bit::autogenesis::{
    ...
    inject_evidence, load_state, save_state, process_turn,
    EvidenceDelta, State,
};
...
async fn post_evidence(
    ...
    match inject_evidence(&mut db, payload.delta, payload.source_uri, payload.meta) {
        Ok(rel_id) => {
            // Persist synchronously internally or via clone to avoid locking file across await.
            let db_clone = db.clone();
            // Free the lock implicitly but it's okay we hold it for file save.
            if let Err(e) = save_state(&app.path, &db_clone) {
