...
async fn main() -> Result<()> {
    ...
    if args.interactive {
        ...
        loop {
            ...
            run_turn(prompt.to_string(), &mut core, &state_path, &receipts_path, &mut history).await?;
        }
    } else if let Some(prompt) = args.prompt {
        let mut history: Vec<serde_json::Value> = Vec::new();
        run_turn(prompt, &mut core, &state_path, &receipts_path, &mut history).await?;
    } else {
...
async fn reason_about_scoring(
    ...
    let system = "You are the meta-reasoning layer of N★ Bit, a live cognitive externalisation engine.\n\
        Your job is to analyze the scoring rule's failure pattern and determine if it needs to change.\n\
        Because you operate under adversarial pressure, discoveries here are empirical research findings.\n\
        Be specific. Reference actual edge pairs and counts from the data. Tensions are research output, not bugs.\n\
        Only recommend a rule change if you can identify a structural flaw from the scorecard.\n\
        Do NOT recommend changes just because a new turn arrived. If evidence is lacking, confidence must be 0.\n\
...
async fn run_turn(
    ...
    // Detect explicit user correction — flag it so the LM can update evidence
    let is_correction = !history.is_empty() && ["no,", "wrong", "that's not", "incorrect",
        "not right", "you're wrong", "that isn't", "no that", "no -", "no—"]
        .iter().any(|pat| prompt.to_lowercase().contains(pat));
    let prompt = if is_correction {
        format!("[USER CORRECTION on previous response] {}", prompt)
    } else {
        prompt
    };

    let rule_reasoning = if core.state.graph.rule_scorecard.is_some() {
        eprintln!("[canonical] scoring meta-pass...");
        let r = reason_about_scoring(&lm, core, receipts_path).await;
        if !r.is_empty() {
            eprintln!("  [reasoning] {}", &r[..r.len().min(120)]);
        }
        r
    } else {
        String::new()
    ...
    let system = format!(
        "You are the N★ Bit canonical execution model. You are a cognitive externalisation engine and a live research instrument.\n\
         Your job is to discover what cognition actually requires from first principles under adversarial pressure.\n\
         Tensions are research output, not bugs. Discovered concepts are load-bearing structural attack vectors.\n\
         \n\
    ...
    // Update history (cap at 20 turns = 40 messages)
    history.push(serde_json::json!({"role":"user","content":prompt}));
    history.push(serde_json::json!({"role":"assistant","content":outs.response.clone()}));
    if history.len() > 40 { history.drain(0..2); }
    ...
    let gate_ok = if result.trace.invariants.passed { "CLEAR" } else { "BLOCKED" };
    let discovered = result.discovered_nodes.len();
    let violations = result.trace.invariants.violations.len();
    println!(
        "[{gate} | +{disc} nodes | nodes={n} | {hash}]",
        gate = gate_ok,
        disc = discovered,
        n = core.state.graph.nodes.len(),
        hash = &result.receipt.hash[..8],
    );
    if violations > 0 {
...
fn populate_seed_queue(core: &mut CanonicalCore) {
    ...
    if !new_pairs.is_empty() {
        eprintln!("  [seeds] +{} new investigation prompts", new_pairs.len());
...
async fn reflect_new_nodes(
    ...
    let system = "You are the predicate discovery layer of N★ Bit, a cognitive externalisation engine.\n\
        ...
        Because this is a live research instrument, these predicates should capture structural attack vectors, epistemic strategies, or open tensions.\n\
        \n\
        Rules:\n\
