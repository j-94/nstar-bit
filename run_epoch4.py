#!/usr/bin/env python3
"""Epoch 4 autogenesis runner — causal_anchor experiment.

Usage:
  python3 run_epoch4.py
  python3 run_epoch4.py --resume
  python3 run_epoch4.py --state-file FILE
  python3 run_epoch4.py --tasks-file FILE
  python3 run_epoch4.py --summary-file FILE
"""
...
parser.add_argument("--state-file", default="nstar-autogenesis/epoch4_fork.json")
parser.add_argument("--tasks-file", default="epoch4_tasks.txt")
parser.add_argument("--summary-file", default="epoch4_summary.tsv")
...
if args.resume:
    ...
    m = state_metrics()
    print(f"Starting epoch 4  turn={baseline_turn}  run={m.get('run_id','?')}")
...
# ── LM-authored epoch 5 fork proposal ─────────────────────────────────────────

print(f"\n{SEP}")
print("EPOCH 5 FORK PROPOSAL (LM-authored)")
print(SEP)
subprocess.run(autogenesis_cmd(
    "propose",
    "--fork-output", "nstar-autogenesis/epoch5_fork.json",
))
